...
import ModalLoadingUpdatePost from './components/ModalLoadingCreate/ModalLoadingCreate';
const initUpdatePostPost: IPayloadCreatePost = {
  target: '',
  type: '',
  description: '',
  medias: []
...
const schemaUpdatePost = yup.object().shape({
...
const UpdatePost = (props: { type: string }) => {
  ...
  const handleOnSubmit = async (data: IPayloadCreatePost) => {
    ...
    await dispatch(action)
      ...
      .then(() => {
        reset();
        toastMessage.success('UpdatePost post success !');
      });
  };

  useEffect(() => {
    if (isLoading) dispatch(openModal({ modalId: MODAL_IDS.loading, dialogComponent: <ModalLoadingUpdatePost /> }));
    else {
      dispatch(closeModal({ modalId: MODAL_IDS.loading, dialogComponent: <ModalLoadingUpdatePost /> }));
    }
